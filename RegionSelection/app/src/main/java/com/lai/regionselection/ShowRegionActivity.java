...
public class ShowRegionActivity extends Activity {
    ...
    private void initDatas() {
        try {
            JSONArray jsonArray = jsonObject.getJSONObject("result").getJSONArray("nation_list");//获取整个json数据
            ...
            for (int i = 0; i < jsonArray.length(); i++) {
                ...
                try {
                    jsonCity = jsonP.getJSONArray("city_list");//在所有的省中取出所有的市，转jsonArray
                } catch (Exception e) {
                    continue;
                }
                //所有的市
                ...
                for (int c = 0; c < jsonCity.length(); c++) {
                    ...
                    try {
                        jsonArea = jsonCy.getJSONArray("town_list");//在从所有的市里面取出所有的区,转jsonArray
                    } catch (Exception e) {
                        continue;
                    }
                    ...
                    for (int a = 0; a < jsonArea.length(); a++) {
                        ...
                        String areaStr = jsonAa.getString("name");//获取所有的区
                        allArea[a] = areaStr;//封装起来
                        Log.i("ssssssssssss",areaStr);
                    }
    ...
    private void initJsonData() {
        try {
            ...
            InputStream is = getAssets().open("area.json");//打开json数据
